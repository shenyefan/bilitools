...
class WatchVideoTask {
     ...
     * 获取推荐视频列表
     */
    async getRecommendVideos(count) {
        try {
            const response = await this.httpClient.get('https://api.bilibili.com/x/web-interface/index/top/rcmd', {
                searchParams: {
                    fresh_type: 3,
                    version: 1,
                    ps: count
            ...
            return response.data.item.filter(video => video.duration > 60 && // 过滤掉太短的视频
    ...
    async getVideoDetail(bvid) {
        try {
            const response = await this.httpClient.get('https://api.bilibili.com/x/web-interface/view', {
                searchParams: { bvid }
            });
            if (response.code !== 0) {
     ...
     * 开始观看视频
     */
    async startWatch(video) {
        try {
            await this.httpClient.postForm('https://api.bilibili.com/x/click-interface/click/web/h5', {
                aid: video.aid,
     ...
     * 模拟观看过程
     */
    async simulateWatching(video, duration) {
     ...
     * 上报观看进度
     */
    async reportWatchProgress(video, watchDuration) {
        try {
