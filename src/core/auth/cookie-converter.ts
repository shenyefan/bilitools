 ...
 */
export async function cookieToToken(cookie: string): Promise<string | undefined> {
  try {
    // 设置cookie
 ...
 */
export async function tokenToCookie(accessToken: string): Promise<string | undefined> {
  try {
    // 使用access_token确认二维码
