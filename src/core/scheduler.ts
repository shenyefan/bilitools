...
class TaskScheduler {
   ...
   * 延迟执行（随机延迟）
   */
  private async delay(maxSeconds: number): Promise<void> {
    if (maxSeconds > 0) {
   ...
   * 执行每日任务
   */
  public async executeDailyTasks(): Promise<SchedulerResult> {
   ...
   * 执行单个任务
   */
  private async executeTask(taskName: string, taskFn: () => Promise<TaskResult>): Promise<TaskResult> {
    try {
      const startTime = Date.now();
   ...
   * 检查任务是否可以执行
   */
  public canExecuteTasks(): boolean {
    try {
      // 检查配置是否有效
      ...
      // 检查是否有启用的任务
      const hasEnabledTasks =
        this.configManager.isTaskEnabled('coin') ||
